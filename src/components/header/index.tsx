...
export default function Header() {
    ...
    return (
        <header className="w-full relative z-10 overflow-hidden h-[730px] max-sm:h-[700px] flex flex-col justify-center items-center">
            ...
            {data.map((src, index) => (
                <Image key={index} src={src} className={`!w-full !h-full absolute inset-0 object-cover object-center zooming-image transition-opacity duration-1000 ease-in-out ${fitImage === index ? 'opacity-100 z-0' : 'opacity-0'}`} alt={`Background ${index}`} width={1020} height={20} />))}
            <span className="z-20 flex w-[50%] max-sm:w-full relative top-[20px] flex-col gap-[30px] max-sm:gap-5 justify-center items-center max-sm:items-center">
                ...
                <button className="px-16  cursor-pointer text-white text-[18px] rounded-[8px] py-3 bg-gradient-to-r from-blue-600 to-blue-300
                 ">
                    Explore More
                </button>
            </span>
